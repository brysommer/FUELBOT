...
import { Driver, FuelRecord } from '@prisma/client';

const adminBotFunction = () => {
    adminBot.onText(/\/start/, async (msg: Message) => {
        await adminBot.setMyCommands([{ command: '/start', description: 'Почати спочатку' }]);
        ...
        const keyboard = {
            reply_markup: {
                inline_keyboard: drivers.map((driver: Driver) => [
                    {
    ...
    adminBot.on('callback_query', async (query: CallbackQuery) => {
                        ...
                        reply_markup: {
                            inline_keyboard: [
                                [
                                    ...
                                    {
                                        text: '📊 Весь період',
                                        callback_data: `stats_all_${driverId}`,
                                    },
                                ],
                            ],
                        },
        ...
        if (data.startsWith('stats_')) {
            ...
            const liters = records.reduce((sum: number, r: FuelRecord) => sum + r.volume, 0);
            const cost = records.reduce((sum: number, r: FuelRecord) => sum + r.total, 0);
            ...
            await adminBot.sendMessage(
                chatId,
                `📊 Статистика (${
                    period === 'week' ? 'Тиждень' : period === 'month' ? 'Місяць' : 'Весь період'
                })  
                    ...
                    reply_markup: {
                        inline_keyboard: [
                            [
                                {
                                    text: '📊 Вивести у CSV',
                                    callback_data: `export_csv_${period}_${driverId}`,
                                },
                            ],
                        ],
                    },
